...
pub async fn run(receipt: &Path, key: &Path, key_id: &str, json: bool) -> Result<()> {
    ...
    let parsed: AnswerReceipt = serde_json::from_str(&body).context("parse receipt JSON")?;
    let raw = fs::read(key)
        .await
    ...
    let v = ReceiptVerifier::from_raw_bytes(key_id, &raw).map_err(|e| anyhow!("verifier: {e}"))?;
