...
impl StorageAdapter for PostgresAdapter {
    ...
    async fn migrate(&self) -> Result<()> {
        let stmts = [
            "CREATE TABLE IF NOT EXISTS pagebridge_nodes (
                ...
                section_aliases JSONB NOT NULL DEFAULT '[]'::jsonb,
                search_vec tsvector
            )",
            ...
            "CREATE INDEX IF NOT EXISTS idx_pb_nodes_canonical ON pagebridge_nodes(doc_id, canonical_section)",
            ...
            // Incremental migrations: ADD COLUMN IF NOT EXISTS (Postgres 9.6+).
            "ALTER TABLE pagebridge_nodes ADD COLUMN IF NOT EXISTS canonical_section TEXT",
            "ALTER TABLE pagebridge_nodes ADD COLUMN IF NOT EXISTS section_aliases JSONB NOT NULL DEFAULT '[]'::jsonb",
            "ALTER TABLE pagebridge_docs ADD COLUMN IF NOT EXISTS raw_text_hash BYTEA",
            "ALTER TABLE pagebridge_docs ADD COLUMN IF NOT EXISTS structural_hash BYTEA",
            "ALTER TABLE pagebridge_docs ADD COLUMN IF NOT EXISTS document_type TEXT",
        ];
        for s in stmts {
            sqlx::query(s)
                .execute(&self.pool)
    ...
    async fn upsert_node(&self, node: &NodeRecord) -> Result<()> {
        ...
        let section_aliases_json = sqlx::types::Json(node.section_aliases.clone());
        ...
        sqlx::query(
            "INSERT INTO pagebridge_nodes (
                ...
                updated_at, source_hash, canonical_section, section_aliases, search_vec
    ...
    async fn find_nodes_by_canonical(
        ...
        // Step 1: matching section node IDs.
        let sec_rows = sqlx::query(
            "SELECT node_id FROM pagebridge_nodes
             WHERE doc_id = $1 AND LOWER(canonical_section) = $2",
        )
        .bind(doc_id.as_str())
        ...
        for row in sec_rows {
            ...
            let leaf_rows = sqlx::query(
                "SELECT * FROM pagebridge_nodes
                 WHERE doc_id = $1 AND is_leaf = TRUE
                   AND (node_id = $2 OR node_id LIKE $3)
                 ORDER BY node_id",
            )
            .bind(doc_id.as_str())
            .bind(&sec_id_str)
